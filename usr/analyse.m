...
syms chi0 Dchi0 Drho0 rho0 g0 D D0 d0 eta0 Ls0 l0 C0
...
eq2W = W0 - D/(rho0*W0/(Dchi0*Drho0*g0)) == 0;
W0i  = solve(eq2W,W0);
W0i  = W0i(1);
...
syms chi0 Dchi0 Drho0 rho0 g0 D0 d0 eta0 Ls0 l0 fReL fRel etae Ri0
...
for eta0=10.^linspace(-1,5,4)
    ...
    % settling diffusivity
    ks0   = w0l.*l0;

    % settling noise
    taus0 = l0./2./w0l;
    xis0  = sqrt(chi0.*ks0./taus0);

    % dimensionless numbers
    Rc0   = W0l./w0l;                % Convection number
    Ra0   = W0l.*D0./ks0;            % Rayleigh number
    ReD0  = W0l.*D0./(eta0./rho0);   % Domain Reynolds number
    ...
    subplot(2,2,2)
    p12 = loglog(D (1,:),W0l(indd,:),'--','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*blk); axis tight; box on; hold on
    subplot(2,2,3)
    ...
    subplot(2,2,4)
    p15 = loglog(D (1,:),Ra0(indd,:),'--','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*prp); axis tight; box on; hold on
    p16 = loglog(D (1,:),ReD0(indd,:),'--','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*grn);
    p17 = loglog(D (1,:),Rc0(indd,:),'--','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*red); axis tight; box on; hold on

    set(0,'CurrentFigure',fig2)
    subplot(2,2,1)
    p21 = loglog(d0(:,1),ks0(:,indD),'--','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*blu); axis tight; box on; hold on
    % subplot(2,2,1)
    % p22 = loglog(d0(:,1),kx0(:,indD),'--','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*grn); axis tight; box on; hold on
    % subplot(2,2,2)
    % p23 = loglog(D (1,:),kx0(indd,:),'--','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*grn); axis tight; box on; hold on
    subplot(2,2,3)
    p24 = loglog(d0(:,1),xis0(:,indD),'--','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*blu); axis tight; box on; hold on
    % subplot(2,2,3)
    % p25 = loglog(d0(:,1),xis0(:,indD),'--','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*grn); axis tight; box on; hold on
    % subplot(2,2,4)
    % p26 = loglog(D (1,:),xis0(indd,:),'--','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*grn); axis tight; box on; hold on
    ...
    xis0  = sqrt(chi0.*ks0./taus0);
    xie0  = sqrt(      ke0./taue0);
    xix0  = sqrt(chi0.*ke0./taue0.*St0./(1+St0.^2));

    % dimensionless numbers for non-turbulent case
    Rc0   = W0i./w0t;                % Turbulent Convection number
    Ra0   = W0i.*D0./kx0;            % Turbulent Rayleigh number
    ReD0  = W0i.*D0./(etae0./rho0);  % Turbulent Domain Reynolds number
    ...
    subplot(2,2,2)
    p32 = loglog(D (1,:),W0i(indd,:),'-.','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*blk); axis tight; box on; hold on
    % p33 = loglog(D (1,:),W0t(indd,:),'-.','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*blk); axis tight; box on; hold on
    subplot(2,2,3)
    ...
    subplot(2,2,4)
    p35 = loglog(D (1,:),Ra0(indd,:),'-.','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*prp); axis tight; box on; hold on
    p36 = loglog(D (1,:),ReD0(indd,:),'-.','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*grn);
    p37 = loglog(D (1,:),Rc0(indd,:),'-.','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*red); axis tight; box on; hold on
    ...
    subplot(2,2,2)
    p43 = loglog(D (1,:),ke0(indd,:),'-.','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*red); axis tight; box on; hold on
    p44 = loglog(D (1,:),kx0(indd,:),'-.','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*grn); axis tight; box on; hold on
    subplot(2,2,3)
    ...
    subplot(2,2,4)
    p47 = loglog(D (1,:),xie0(indd,:),'-.','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*red); axis tight; box on; hold on
    p48 = loglog(D (1,:),xix0(indd,:),'-.','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*grn); axis tight; box on; hold on
    ...
    while res>tol
        ...
        res = norm(W0prv-W0)./norm(W0) + norm(w0prv-w0)./norm(w0);
    end
    fRel0 = double(fRel0);
    fReL0 = double(fReL0);
    ...
    ks0   = w0.*l0;
    ke0   = W0./D0.*L0.^2;
    etas0 = eta0 + fRel0.*rho0.*ks0;
    etae0 = eta0 + fReL0.*rho0.*ke0;
    ...
    St0   = txi0./taue0;
    xis0  = sqrt(chi0       .*ks0./taus0);
    xie0  = sqrt(      fReL0.*ke0./taue0);
    xix0  = sqrt(chi0.*fReL0.*ke0./taue0.*St0./(1+St0.^2));

    % dimensionless numbers for non-turbulent case
    Rc0   = W0./w0;                    % General Convection number
    Ra0   = W0.*D0./kx0;               % General Rayleigh number
    ReD0  = W0.*D0./(etae0./rho0);     % General Domain Reynolds number
    ...
    subplot(2,2,2)
    p1(etait) = loglog(D (1,:),W0(indd,:),'-','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*blk); axis tight; box on; hold on
    subplot(2,2,3)
    ...
    subplot(2,2,4)
    p55 = loglog(D (1,:),Ra0(indd,:),'-','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*prp); axis tight; box on; hold on
    p56 = loglog(D (1,:),ReD0(indd,:),'-','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*grn);
    p57 = loglog(D (1,:),Rc0(indd,:),'-','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*red); axis tight; box on; hold on
    drawnow;
    ...
    subplot(2,2,2)
    p2(etait) = loglog(D (1,:),(fReL0(indd,:).*ke0(indd,:)),'-','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*red); axis tight; box on; hold on
    p64       = loglog(D (1,:),kx0(indd,:),'-','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*grn); axis tight; box on; hold on
    subplot(2,2,3)
    ...
    subplot(2,2,4)
    p67 = loglog(D (1,:),xie0(indd,:),'-','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*red); axis tight; box on; hold on
    p68 = loglog(D (1,:),xix0(indd,:),'-','LineWidth',1.5,'Color',lnshd.*wht + (1-lnshd).*grn); axis tight; box on; hold on
    drawnow;

    set(0,'CurrentFigure',fig3); clf;
    subplot(2,2,1);
    imagesc(log10(D (1,:)),log10(d0(:,1)),log10(Rc0)); axis xy tight; box on; hold on; colorbar('Ticklabelinterpreter','latex','FontSize',11); colormap(colmap);
    clim([-1,1].*max(abs(log10(Rc0(:)))));
    subplot(2,2,2);
    imagesc(log10(D (1,:)),log10(d0(:,1)),log10(Red0)); axis xy tight; box on; hold on; colorbar('Ticklabelinterpreter','latex','FontSize',11); colormap(colmap);
    clim([-1,1].*max(abs(log10(Red0(:)))));
    subplot(2,2,3);
    imagesc(log10(D (1,:)),log10(d0(:,1)),log10(Ra0)); axis xy tight; box on; hold on; colorbar('Ticklabelinterpreter','latex','FontSize',11); colormap(colmap);
    ...
    Ics = zeros(size(D0));
    Ics(Ra0<=10 & ReD0 <= 10) = 1;
    Ics(Ra0> 10 & ReD0 <= 10) = 2;
    Ics(Ra0<=10 & ReD0 >  10) = 3;
    Ics(Ra0> 10 & ReD0 >  10) = 4;

    set(0,'CurrentFigure',fig4); clf;
    imagesc(log10(D (1,:)),log10(d0(:,1)),Ics); axis xy tight; box on; hold on; colormap(colmap); clim([1 4])
...
legend([p11,p31,p51],{'laminar','inertial','general'},'Interpreter','latex','FontSize',11,'Location','southeast');
...
text(0.02,0.91,'\textbf{(a)}','Interpreter','latex','FontSize',13,'Units','normalized')
legend([p21,p41,p61],{'laminar','inertial','general'},'Interpreter','latex','FontSize',11,'Location','southeast');
