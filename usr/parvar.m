...
D     = [3e+0; 1e+1 (ref); 1e+2;]; % [m]
d0    = [1e-4; 1e-3; 1e-2 (ref);]; % [m]
etam0 = [1e-1; 1e+1 (ref); 1e+3]; % [Pas]

% additional parameter variations on reference run (12 simulations, TK)
N          = 200;    % [100; 300];
CFL        = 0.5;    % [0.25; 1]
elle       = h/2;    % [1/4; 2]*h
ells       = 10*d0;  % [5; 20]*d0
...
Xi         = 1.0;    % [0.1]
xeq        = 0.01;   % [0.1]
d0         = 0.03;   % D = 3; etam0 = 1e1; (settling regime)
...
% additional parameter variations on magma ocean ref run (5 simulations
gamma      = 1e-3;   % [0,1e-2];
elle       = h/2;    % [h/4,2*h,4*h];

% total # simulations: 27+12+4+5 = 48
